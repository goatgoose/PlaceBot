class Color:
    ...
    @staticmethod
    def colors_by_name():
        return {c.name: c for c in Color.colors_by_id().values()}

    @staticmethod
    def colors_by_hex():
        return {c.hex: c for c in Color.colors_by_id().values()}

    @staticmethod
    def from_pixel(rgb_im_px) -> 'Color':
        r, g, b = rgb_im_px
        hex_ = Color.rgb2hex(r, g, b)
        return Color.colors_by_hex()[hex_.upper().strip("#")]

    @staticmethod
    def from_name(name: str):
        return Color.colors_by_name().get(name.upper())
